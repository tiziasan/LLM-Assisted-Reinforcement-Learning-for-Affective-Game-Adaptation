class Player {
  ...
  initMembers = game => {
    setTimeout(function() {
      window.location.href = 'ExperimentOver.html'
      this.sendInfo();
    }, 600000);
  ...
  initListeners = () => {
    ...
    const kGoal = keyboard('Escape')
    const goal = keyboard('t')

    nextWeapon.release = () => this.weapons.next()
    prevWeapon.release = () => this.weapons.prev()

    kGoal.press = goal.press = () => activateGoalDOM()
    kGoal.release = goal.release = () => deactivateGoalDOM()
  ...
  damage = zombie => {
    ...
    if (this.health <= 0 && !this.sendData) {
      this.health = 0;
  ...
  sendInfo = () => {
      ...
      const Data = { 
        level: this.game.zombieManager.getLevel().toString(),
        kills: this.game.zombieManager.getKilledZombies().toString(), 
      };
     this.postHandler.postData(Data);
